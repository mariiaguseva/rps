...
class SortingApp:
    ...
    def create_main_interface(self):
        ...
        ttk.Label(input_frame, text="Массив (через запятую):").pack(anchor="w", pady=(10, 0))
        ...
        ttk.Button(save_frame, text="Сохранить исходный массив",
                   command=lambda: self.save_array('original')).pack(side="left", padx=5)
        ttk.Button(save_frame, text="Сохранить отсортированный массив",
                   command=lambda: self.save_array('sorted')).pack(side="left", padx=5)
    ...
    def save_array(self, array_type):
        if not self.current_user:
            messagebox.showwarning("Предупреждение", "Сначала авторизуйтесь")
            return

        if array_type == 'original' and not self.original_array:
            messagebox.showwarning("Предупреждение", "Нет исходного массива для сохранения")
            return
        elif array_type == 'sorted' and not self.sorted_array:
            messagebox.showwarning("Предупреждение", "Сначала отсортируйте массив")
            return

        array_to_save = self.original_array if array_type == 'original' else self.sorted_array
        sorted_array = self.sorted_array if array_type == 'sorted' else []

        success, message = db_manager.save_array(
            self.current_user['id'],
            array_to_save,
            sorted_array if sorted_array else array_to_save
        )

        if success:
            messagebox.showinfo("Результат", message)
            self.status_var.set(f"Массив сохранен в базу данных")
        else:
            messagebox.showerror("Ошибка", message)
